...
Here we only comment on the difference in observations from the previous graph. These observations appear to reasonably explain the given specifications. However, we see in the line for the Fourth Year Individuals that it exceeds the population dynamics line in the twelfth year. Given that we are told to use a "[detection] probability of p=0.5 and no double counting", this does not seem like as good model a model as the binomial distribution. We encounter this as the poisson distribution has no explicit upper bound and so a value such as this, however unlikely, can be obtained. Whereas the binomial distribution is bounded by the number of trials (in our case the abundance for class $i$ at time $t$). We conclude that the binomial distribution must be a better model for the observations than the poisson distribution.
